 ...
 *
 * Radix wraps whatever you put in a viewport in its own
 * `<div style="min-width:100%; display:table">`. A table box is shrink-to-fit:
 * `min-width` gives it a floor, but long content makes it grow *wider* than the
 * viewport rather than wrap — which quietly breaks every `truncate` in the cart
 * lines and adds a horizontal scrollbar nobody asked for. Forcing that wrapper
 * back to `block` restores normal width behaviour.
 *
 * The thumb is retinted because the shadcn default (`bg-border`) is a hairline
 * grey that disappears against this page's white panels.
 */
const PANEL_SCROLL = cn(
  "[&>[data-slot=scroll-area-viewport]>div]:!block",
  "[&_[data-slot=scroll-area-thumb]]:bg-black/25"
)
...
const CartLine = ({
  ...
  <li className="flex items-stretch gap-4 py-3 last:border-b-0">
    ...
    <div className="flex min-w-0 flex-1 flex-col justify-center gap-1">
      ...
      <p className={cn("truncate text-xs text-black/60")}>
    ...
    <button
      ...
      className="h-fit shrink-0 self-center rounded-full bg-white p-2 transition-colors hover:bg-red-700 hover:text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-black"
    >
...
export const CartDialog = ({
  ...
  const handleOpenChange = (next: boolean) => {
    console.log("handleOpenChange", next)
        ...
        >
          <Dialog.Title asChild>
            <div className="flex shrink-0 items-center justify-between gap-4 border-b-2 border-black bg-amber-400 px-6 py-4">
              ...
              <Dialog.Close
                aria-label="Close"
                className="rounded-full p-2 transition-colors hover:bg-black hover:text-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-black"
              >
          ...
          {/* The mask clips the slide. Vertical scrolling belongs to each
              panel's own ScrollArea, not here — two scrollports on one element
              fight each other. */}
          <div className="min-h-0 flex-1 overflow-hidden">
            <div
              className={cn(
                ...
                step === "cart" ? "translate-x-0" : "-translate-x-1/2"
              )}
            >
              {/* ── panel one: the cart ── */}
              <ScrollArea
                className={cn("max-h-125 w-1/2")}
                // inert={step === "form"}
              >
                <div className="py-2">
                  {items.length === 0 ? (
                    ...
                    groups.map((group) => (
                      ...
                      >
                        <Link
                          ...
                          className="ml-6 bricolage-grotesque-500 mt-3 inline-flex items-baseline gap-2 border-b border-b-transparent bg-white transition-colors hover:border-b-black focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-black"
                        >
                          {/* Go to{" "} */}
                          {group.propertyName}{" "}
              ...
              <ScrollArea
                className={cn("max-h-125 w-1/2", PANEL_SCROLL)}
                // inert={step === "cart"}
              >
          ...
          <div className="flex shrink-0 flex-wrap items-center justify-between gap-4 border-t-2 border-black bg-amber-100 px-4 py-4 pl-6">
            <div>
